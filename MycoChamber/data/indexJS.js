function updateTimers() {
  ...
  var url = "/timer?"+components[0].name+"="+components[0].value+"&"+
            ...
            components[2].name+"="+components[2].value+"&"+
            components[3].name+"="+components[3].value+"&"+
            components[4].name+"="+components[4].value+"&"+
            components[5].name+"="+components[5].value;
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      var newValues = JSON.parse(this.responseText);
      document.getElementById("fae-values").innerHTML = "ON: " + newValues.newON1 + " ms\nOFF: " + newValues.newOFF2 + " ms";
      document.getElementById("led-values").innerHTML = "ON: " + newValues.newON3 + " ms\nOFF: " + newValues.newOFF4 + " ms";
      document.getElementById("mister-values").innerHTML = "ON: " + newValues.newON5 + " \u0025 RH\nOFF: " + newValues.newOFF6 + " \u0025 RH";
    }
  };
  xhr.open("POST", url);
